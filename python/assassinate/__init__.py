"""Assassinate - Python interface to Metasploit Framework.
...
Assassinate provides a Pythonic API for interacting with Metasploit Framework
...
try:
    ...
    def _not_built(*args, **kwargs):
        raise ImportError(
            "The assassinate Rust module is not built. Run one of:\n"
    ...
    job_info = _not_built
    job_kill = _not_built
    job_wait_for_session = _not_built
...
from .log_config import setup_from_config
setup_from_config()
...
def wait_for_new_session(
    ...
    """
    Wait for a new session to appear by polling list_sessions().
    ...
    WARNING: This polling approach has race conditions - sessions may appear
    before bootstrap completes. For reliable job-based session waiting, use
    `job_wait_for_session(job_id)` instead, which uses MSF's native
    session_waiter_event mechanism.
    ...
    Example:
        # For job-based exploits, prefer job_wait_for_session:
        job_id = exploit.exploit("cmd/unix/interact", job=True)
        session_id = job_wait_for_session(job_id, timeout_secs=30)
        session = get_session(session_id)

        # This polling function is for edge cases only:
        existing = set(list_sessions())
        # ... trigger session creation ...
        session = wait_for_new_session(existing_sessions=existing)
    """
    logger.debug(
        f"wait_for_new_session() called (timeout_ms={timeout_ms}, interval_ms={interval_ms})"
    )

    effective_timeout_ms = timeout_ms or 60000
    effective_interval_ms = interval_ms or 100
    ...
    logger.info(f"Waiting for new session (timeout: {effective_timeout_ms}ms)...")
    found = _poll_releasing_gvl(check_for_session, effective_interval_ms, effective_timeout_ms)

    if not found or new_session_id[0] is None:
        logger.warning(
